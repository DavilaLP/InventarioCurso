...
const SidebarItem = ({item, isOpen, closeSidebar}) => {
  ...
  if (item.childrens){
        ...
        {item.childrens.map((child, index) => {
           return(
             <div key={index} className='s-child'>
                <NavLink to={child.path} className={activeSublink} onClick={handleLinkClick}>
    ...
    return  (
      <NavLink to={item.path} className={activeLink} onClick={handleLinkClick}>
